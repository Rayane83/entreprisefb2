...
# 🚀 LANCEUR PRODUCTION CORRIGÉ - flashbackfa-entreprise.fr
# Versions packages corrigées + génération automatique secrets
...
# Fonction: Installation dépendances Python avec versions compatibles
install_python_deps() {
    echo -e "${BLUE}Installation dépendances Python (versions compatibles)...${NC}"
    cd "$BACKEND_DIR"
    
    source "$VENV_DIR/bin/activate"
    
    echo "Installation par étapes avec versions testées..."
    
    # Installer packages individuellement avec versions compatibles
    ...
    echo "  ✅ Python-multipart installé"
    
    # Installer cryptography avec version disponible
    pip install "cryptography>=41.0.0,<42.0.0" --quiet
    echo "  ✅ Cryptography installé (version compatible)"
    
    # Installer python-jose avec cryptography
    pip install "python-jose[cryptography]>=3.3.0" --quiet
    echo "  ✅ Python-jose installé"
    
    # Installer bcrypt et passlib
    pip install bcrypt>=4.0.0 --quiet
    echo "  ✅ Bcrypt installé"
    
    pip install "passlib[bcrypt]>=1.7.4" --quiet
    echo "  ✅ Passlib installé"
    
    echo -e "${GREEN}✅ Toutes les dépendances Python installées${NC}"
}
...
# Fonction: Test des imports Python
test_python_imports() {
    ...
    if python -c "
import fastapi, uvicorn, sqlalchemy, pymysql, cryptography
...
" 2>/dev/null; then
        echo -e "${GREEN}✅ Tous les imports Python validés${NC}"
        return 0
    else
        echo -e "${YELLOW}⚠️  Problème avec certains imports${NC}"
        return 1
    fi
}
